...
if __name__ == "__main__":
    ...
    # Prepare the active learning agent to make sure that your credentials are correct
    ...
    sampler_config = SamplerConfig(method=SamplingMethod.CORAL, n_samples=45, name="CORAL_15")
    al_agent.query(sampler_config=sampler_config, al_scorer=scorer)

    # Download the new samples via the CLI and copy the corresponding images to the labeled folder.
    ...
    cli_command = f"lightly-download token={lighty_webapp_token} dataset_id={lighty_webapp_dataset_id}" \
                  f"tag_name=CORAL_15 input_dir={path_full_dataset} output_dir={path_labeled_set}" \
